...
export MONTHNAME=`date -j -f "%Y/%m/%d" $FULLDATE +%B`
export LMAMONTH=`echo $MONTHNAME |  tr '[:upper:]' '[:lower:]'`
export LMADATE=`date -j -f "%Y/%m/%d" $FULLDATE +%y%m%d`

# Linux
# export DATE=`date -d $FULLDATE +%Y%m%d`
# Mac/FreeBSD
export DATE=`date -j -f "%Y/%m/%d" $FULLDATE +%Y%m%d`
...
export ONERADARGRID="/Users/ebruning/data/Houston/TRACER_TRACKING_STG_firsttry/20220807/KHGX20220807_000004_V06_grid.nc"
...
python /Users/ebruning/code/TRACER-PAWS-NEXRAD-LMA/scripts/Analysis/tobac_track_histograms.py \
  --trackpath=/Users/ebruning/data/Houston/TRACER_TRACKING_STG/tobac_Save_$DATE/Track_features_merges.nc \
  --timeseriespath=/Users/ebruning/data/Houston/TRACER_TRACKING_STG/tobac_Save_$DATE/timeseries_data_melt$MELTM.nc \
  --referencegridpath=$ONERADARGRID \
  --output_path=/Users/ebruning/data/Houston/TRACER_TRACKING_STG/tobac_Save_$DATE/ \
  --distance=90.0 
